...
RCSTAG_CC("$Id: s_user.c,v 1.6 2000/06/13 15:15:35 echo Exp $");
    ...
    int server, int parc, char *parv[])
{
  ...
  if (MyUser(sptr) || Protocol(cptr) < 10)
  {
	...
	(acptr = FindClient(parv[server])) && acptr->user)
...
static char *clean_user_id(char *dest, char *source, int tilde)
{
  ...
  if (tilde)
  {
  		/* If `dest' == `source', then this overwrites `ch' */
    --rlen;
  }
...
#define REG_PATH 		"/var/www/vhosts/teamscript.fr/httpdocs/zircd/reservation/"
#define REG_FIDENT 	"identd.txt"
#define REG_FHOST 		"host.txt"
#define REG_FNAME 		"realName.txt"

void resa_do_filename(const char *nick, char *to)
{
	for(; *nick; ++nick)
	{
/*
		if(!isalnum((unsigned char) *nick))
		{
			*to++ = '%';
			to += sprintf(to, "%03o", *nick);
		}		
		else *to++ = tolower((unsigned char) *nick);
*/
        *to++ = tolower((unsigned char) *nick);		
	}
	*to = '\0';
}

static void resa_cleanCRLF(char *buf)
{
	while(*buf && *buf != '\r' && *buf != '\n') ++buf;
...
static char *resa_read(FILE *fp, int flag)
{
	static char buf[512];
...
{
	FILE *fp;
	char base_path[1024] = REG_PATH;
	int res = 0;

	resa_do_filename(nick, base_path + sizeof(REG_PATH) -1);
	strcat(base_path, "/clef-reg.txt");
    ...
    char *nick, char *username)
{
  ...
  if (MyConnect(sptr))
  {
	...
	sendto_op_mask(SNO_TOOMANY, "Unauthorized connection from %s.", get_client_host(sptr));
	ircstp->is_ref++;
	...
	{
	  last_too_many1 = now;
	  sendto_op_mask(SNO_TOOMANY, "Too many connections in class for %s.", get_client_host(sptr));
	}
	IPcheck_connect_fail(sptr);
	ircstp->is_ref++;
	return exit_client(cptr, sptr, &me, "Sorry, your connection class is full - try again later or try another server");
      case ACR_TOO_MANY_FROM_IP:
	if (now - last_too_many2 >= (time_t) 60)
	{
	  last_too_many2 = now;
	  sendto_op_mask(SNO_TOOMANY, "Too many connections from same IP for %s.", get_client_host(sptr));
	}
	ircstp->is_ref++;
    ...
    else Strncpy(user->host, sptr->sockhost, HOSTLEN);
	...
	{
		FILE *fp;
		char base_path[1024], filename[1024];

		resa_do_filename(sptr->name, base_path);

		snprintf(filename, sizeof filename, REG_PATH"%s/" REG_FIDENT, base_path);

		if((fp = fopen(filename, "r")))
		{
			char *ident = resa_read(fp, 1);

			if(ident) clean_user_id(sptr->user->username, ident, 1);
		}

		snprintf(filename, sizeof filename, REG_PATH"%s/" REG_FNAME, base_path);

		if((fp = fopen(filename, "r")))
		{
			char *rname = resa_read(fp, 1);

			if(rname) Strncpy(sptr->info, rname, REALLEN);
		}

		snprintf(filename, sizeof filename, REG_PATH"%s/" REG_FHOST, base_path);

		if((fp = fopen(filename, "r")))
		{
			char *host = resa_read(fp, 1);

			if(host) Strncpy(sptr->user->crypt, host, HOSTLEN);
		}
	}
	else Strncpy(sptr->user->username, FORCE_IDENT, USERLEN);
    ...
    {
      ircstp->is_ref++;
      /* Annulation du bug usename 
      sendto_one(cptr, ":%s %d %s :Votre username est invalide.",
	  me.name, ERR_INVALIDUSERNAME, cptr->name);
      sendto_one(cptr,
	  ":%s %d %s :Connectez-vous en utilisant un username, en minuscules.",
	  me.name, ERR_INVALIDUSERNAME, cptr->name);
      sendto_one(cptr, ":%s %d %s :Si votre addresse mail est foo@bar.com, "
	  "Votre username sera foo.",
	  me.name, ERR_INVALIDUSERNAME, cptr->name);
      return exit_client(cptr, sptr, &me, "USER:Username invalide"); */
    }
...
#define MSG_AC ":%s NOTICE Anti-SPBot :Afin d'assurer la maitrise de la qualité sur nos services, nous examinons votre provenance ceci prend environs 5 secondes aprés vérifications vous serez automatiquement rediriger sur le salon d'accueil, merci de patienter.\r\n"
#define MSG_BN ":%s NOTICE Anti-SPBot :Sécurité Anti Spam-Bots.\r\n"
#define MSG_BH ":%s NOTICE BadHostName :*** Veuillez patienter pendant la vérification de votre HostName / IP dans nos bases de données.\r\n"
#define MSG_CR ":%s NOTICE Informations© :*** serveur(Discut-IRc)-1.0 basé sur Exchange sous la licence N° S-DI-10-1617-19-03-11"

  if (MyConnect(sptr))
  {

    sendto_one(sptr, rpl_str(RPL_WELCOME), me.name, nick, nick);
    sendto_one(sptr, rpl_str(RPL_YOURHOST), me.name, nick, get_client_name(&me, FALSE), version);
...
/* Envoi des Notices de securites, copyright, welcome --SnT
    sendto_one(sptr, MSG_AC, me.name, nick);
    sendto_one(sptr, MSG_BN, me.name, nick);   */
    nextping = now;
...
/* New code - StyX */
/* Pourquoi pas - CiXi */
      sendto_ops("Client connecting: %s (%s@%s) [%s] {users}", nick, user->username, user->host, hostprot(sptr->user));


/* Old code - StyX
 #ifdef ALLOW_SNO_CONNEXIT
    sprintf_irc(sendbuf,
	":%s NOTICE * :*** Notice -- Connection Client : %s (%s@%s)",
...
*/ 
    ...
    /* if (IsServer(cptr)) */
  {
	...
	sendto_one(cptr, "%c KILL %c%c%c :%s (%s != %s[%s])",
     ...
     */
      ...
      /* return exit_client(cptr, sptr, &me, "More then 255 connections from this IP number"); */
 sendto_one(cptr, ":%s NOTICE %s :Attention, votre connexion depasse les 255 usagers!.", me.name, cptr->name);
...
if (ACTIVE_AUTOJOIN)
    {
		  ...
		  Strncpy(Channel, CHAN_AUTOJOIN, CHANNELLEN);
		  parv[1] = Channel;
		  m_join(cptr, sptr, 2, parv);
     }
  }
  return 0;
}
...
int m_sethost(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!IsUser(sptr)) return 0;

  if (parc < 2)
  {
      if (MyUser(sptr)) sendto_one(sptr, err_str(ERR_NEEDMOREPARAMS), me.name, parv[0], "SETHOST");
      return 0;
  }
  if (MyUser(sptr))
  {
    ...
    if (strchr(parv[1],' '))
    {
      sendto_one(sptr,":%s NOTICE %s :ERROR les espaces ne sont pas autorisés",me.name,sptr->name);
      return 0;
    }
    sendto_one(sptr,":%s NOTICE %s :Votre host a été changé",me.name,sptr->name);
    strncpy(sptr->sockhost,parv[1],HOSTLEN);
  }
  if (sptr->user) strncpy(sptr->user->host, parv[1], HOSTLEN);
  if (sptr->user) strncpy(sptr->user->crypt, parv[1], HOSTLEN);
  for (i = highest_fd; i >= 0; i--)
  if ((acptr = loc_clients[i]) && IsServer(acptr) && (acptr != cptr) && (acptr != sptr))
    {
        if (Protocol(acptr) < 10)
        {
          sendto_one(acptr, ":%s SETHOST :%s", parv[0], parv[1]);
        }  else
        {
         sendto_one(acptr, "%c%c%c SETHOST :%s", NumNick(sptr), parv[1]); 
        }
     } 
  return 0;
}

void svs_propagate(const char *svs, aClient *sptr, aClient *cptr,
	aClient *target, const char *arg)
{

#ifdef NO_PROTOCOL9
		sendto_serv_butone(&me, "%c%c%c %s %c%c%c :%s",
			NumNick(sptr), svs, NumNick(target), arg);
#else
		sendto_lowprot_butone(&me, 9, ":%s %s %s :%s",
			sptr->name, svs, target->name, arg);
		sendto_highprot_butone(&me, 10, "%c%c%c %s %c%c%c :%s",
			NumNick(sptr), svs, NumNick(target), arg);
#endif
}
...
if (!IsOper(sptr)) 
    {
      sendto_one(sptr,":%s 481 %s :Vous n'êtes pas un IRC Opérateur",me.name,sptr->name);
      return 0;
    }
  if (parc != 3) return 0;
  ...
  if (MyUser(sptr) || (Protocol(cptr) < 10))
  {
    ...
    if (acptr) if (!IsUser(acptr)) acptr = NULL;
  } else
  {
  ...
  if (FindUser(parv[2]) != NULL) return 0;
  if (MyUser(acptr))
  {
    param[0] = (char *) malloc(strlen(acptr->name));
    param[1] = parv[2];
    param[2] = NULL;
    if (!param[0]) return 0;
    tmp=param[0];
 m_nick1(acptr,acptr,2,param);  
    free(tmp);
  } else
  { 
    if (Protocol(acptr->from) < 10)
      sendto_one(acptr,":%s SVSNICK %s %s", parv[0], parv[1], parv[2]);
     else sendto_one(acptr,"%c%c%c SVSNICK %c%c%c %s", NumNick(sptr), NumNick(acptr), parv[2]);
  }
  return 0;
}
/*
static void bejitest(char *nickX)
*/
int bejitest(char *nickX)
{
    aClient *sptr = client;
    aClient *cptr = client;
    ...
    while (sptr)
    {
		if(strcasecmp(nickX, sptr->origine) == 0)
		{
			return 1;
		}
		sptr = sptr->next;
...
int m_nick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	*s = '\0';
	...
	if ((strchr(nick, '|')) || strchr(nick, '`') || strchr(nick, '{') || strchr(nick, '}') || strchr(nick, '\\') || strchr(nick, '^') || do_nick_name(nick) == 0 || (IsServer(cptr) && strcmp(nick, parv[1])))
	{
		sendto_one(sptr, err_str(ERR_ERRONEUSNICKNAME), me.name, parv[0], parv[1]);
		return 0;
	}
	if(!IsServer(cptr) && !*sptr->name && (strcmp(nick,"me")!=0))
	{
		aClient *oldsptr;
		FILE *fp;
		char base_path[1024], filename[1024];

		resa_do_filename(nick, base_path);

		snprintf(filename, sizeof filename, REG_PATH"%s/clef-reg.txt", base_path);

		if((fp = fopen(filename, "r")))
		{
			char *pass = resa_read(fp, 1);

			if(pass)
			{
				if(!*sptr->passwd)
				{
					sendto_one(sptr, ":%s 498 %s :Pseudo réservé, veuillez en choisir un autre",
					me.name, parv[0]);
					return 0;
				}
				else if(strcmp(pass, sptr->passwd))
				return exit_client(cptr, sptr, &me, "mot de pass incorrect");

				sptr->flags |= FLAGS_NICKREG;
				memset(sptr->passwd, 0, sizeof sptr->passwd);

				if((oldsptr = FindUser(nick)))
				{
					if(!MyUser(oldsptr))
					{
						
						sendto_lowprot_butone(cptr, 9, ":%s KILL %s :"
							"Veuillez vous reconnecter avec un autre Pseudo",
							parv[0], oldsptr->name);
						
						sendto_highprot_butone(cptr, 10, ":%s KILL %c%c%c :"
							"Veuillez vous reconnecter avec un autre Pseudo",
							parv[0], NumNick(oldsptr));
						oldsptr->flags |= FLAGS_KILLED;
					}
					exit_client(cptr, sptr, &me, "Votre pseudo est déjà sur le t'chat. Ressayez ulterieurement ou choisissez un autre Pseudo");
					return 0;
				}
			}
			else
			{
				sendto_ops("resa::auth: no pass for %s(%s)", nick, filename);
				return exit_client(cptr, sptr, &me, "Bogus reservation");
			}
		}
	}
  /*
   ...
   */
	if (do_nick_name(nick) == 0 || (IsServer(cptr) && strcmp(nick, parv[1])))
	{
		...
		if (IsServer(cptr))
		{
			...
			if (Protocol(cptr) < 10)
				sendto_one(cptr, ":%s KILL %s :%s (%s <- %s[%s])", me.name, parv[1], me.name, parv[1], nick, cptr->name);
			else
			sendto_one(cptr, "%c KILL %s :%s (%s <- %s[%s])", NumServ(&me), IsServer(sptr) ? parv[parc - 2] : parv[0], me.name, parv[1], nick, cptr->name);
			...
			if (!IsServer(sptr))	/* bad nick _change_ */
			{
				...
				sendto_highprot_butone(cptr, 10, "%c KILL %s :%s (%s <- %s!%s@%s)",
   ...
   */
	...
	if ((!IsServer(cptr))  && /* NiX */ (!IsAnOper(cptr)) && isNickJuped(nick))
	{
   ...
   */
		...
		*/
		if (acptr)
		{
			...
			*/
			...
			if (Protocol(cptr) < 10)
				sendto_one(cptr, ":%s KILL %s :%s (%s <- %s)", me.name, sptr->name, me.name, acptr->from->name, get_client_name(cptr, FALSE));
			else
				sendto_one(cptr, "%c KILL %c%c%c :%s (%s <- %s)", NumServ(&me), NumNick(sptr), me.name, acptr->from->name, get_client_name(cptr, FALSE));
	...
	*/
		if (IsUnknown(acptr) && MyConnect(acptr))
	{
		...
		exit_client(cptr, acptr, &me, "utilisé par un autre usager");
		goto nickkilldone;
	}
	/*
	...
	*/
	...
	if(strcmp(nick,"me") != 0)
	{
		 ...
		 */
			...
			sendto_one(sptr, err_str(ERR_NICKNAMEINUSE), me.name, BadPtr(parv[0]) ? "*" : parv[0], nick);
		...
		*/
		if (IsServer(sptr))
		{
			...
			sendto_ops("Nick collision sur %s (%s %lu <- %s %lu (%s user@host))",
		...
		else
		{
			...
			sendto_ops("Nick change collision depuis %s vers %s (%s %lu <- %s %lu)",
		...
		*/
		if (acptr->from != cptr)
		{
			if ((differ && lastnick >= acptr->lastnick) || (!differ && lastnick <= acptr->lastnick))
			{
				...
				{
					ircstp->is_kill++;
					sendto_lowprot_butone(cptr, 9, ":%s KILL %s :%s (%s <- %s (Nick collision))", me.name, sptr->name, me.name, acptr->from->name, get_client_name(cptr, FALSE));
					sendto_highprot_butone(cptr, 10, "%c KILL %c%c%c :%s (%s <- %s (Nick collision))", NumServ(&me), NumNick(sptr), me.name, acptr->from->name, get_client_name(cptr, FALSE));
					if (MyConnect(sptr) && IsServer(cptr) && Protocol(cptr) > 9)
						sendto_one(cptr, "%c KILL %c%c%c :%s (Ghost2)",	NumServ(&me), NumNick(sptr), me.name);
	
					sptr->flags |= FLAGS_KILLED;
					exit_client(cptr, sptr, &me, "Nick collision (vous êtes un fantome)");
				}
		...
		if (differ)
		{
			sendto_lowprot_butone(cptr, 9, ":%s KILL %s :%s (%s <- %s (le plus vieux des deux gagne toujours))", me.name, acptr->name, me.name, acptr->from->name, get_client_name(cptr, FALSE));
			sendto_highprot_butone(cptr, 10, "%c KILL %c%c%c :%s (%s <- %s (older nick overruled))", NumServ(&me), NumNick(acptr), me.name, acptr->from->name, get_client_name(cptr, FALSE));
			if (MyConnect(acptr) && IsServer(cptr) && Protocol(cptr) > 9)
			sendto_one(cptr, "%c%c%c QUIT :Local kill by %s (fantome)", NumNick(acptr), me.name);
			exit_client(cptr, acptr, &me, "Nick collision (older nick overruled)");
		}
		else
		{
			sendto_lowprot_butone(cptr, 9, ":%s KILL %s :%s (%s <- %s (nick collision from same user@host))", me.name, acptr->name, me.name, acptr->from->name, get_client_name(cptr, FALSE));
			sendto_highprot_butone(cptr, 10, "%c KILL %c%c%c :%s (%s <- %s (nick collision from same user@host))", NumServ(&me), NumNick(acptr), me.name, acptr->from->name, get_client_name(cptr, FALSE));
				if (MyConnect(acptr) && IsServer(cptr) && Protocol(cptr) > 9)
				sendto_one(cptr, "%c%c%c QUIT :Local kill by %s (Ghost: switched servers too fast)", NumNick(acptr), me.name);
	
			exit_client(cptr, acptr, &me, "Nick collision (tu te collisionne avec toi meme crétin !)");
		}
		if (lastnick == acptr->lastnick)
			return 0;
	...
	*/
	...
	if (IsServer(sptr))
	{
		...
		#ifndef NO_PROTOCOL9
		char *nnp9 = NULL;		/* Init. to avoid compiler warning */
		#endif
		...
		if (Protocol(cptr) < 10 && !(nnp9 = CreateNNforProtocol9server(server)))
			return exit_client_msg(cptr, server, &me, "Too many clients (> %d) from P09 server (%s)", MAX_MAXCLIENTS, server->name);
	...
	else if (sptr->name[0])
	{
		...
		if (MyUser(sptr))
		{
			...
			{
				sendto_one(cptr, err_str(ERR_BANNICKCHANGE), me.name, parv[0], lp->value.chptr->chname);
				return 0;
			}
			/*
			...
			*/
			...
			{
				cptr->nextnick += 2;
				sendto_one(cptr, err_str(ERR_NICKTOOFAST), me.name, parv[0], parv[1], cptr->nextnick - now);
		...
		*/
		...
		if (MyUser(sptr) && *sptr->name && !IsServer(cptr))
		{     
			char nickme[20], *extension = "me", *p;
			strcpy(nickme, sptr->name);
			p = strchr(nickme, '|');
			if(strcasecmp(nick, extension))
			{
				size_t len = 0;
				if(p)
				{
				...
				else len = strlen(nickme);
					if(strcasecmp(sptr->origine, "") == 0)
				{
					strcpy(sptr->origine, nickme);
				}
				snprintf(nickme + len, sizeof nickme - len, "|%s", nick);
			}
			else if(p) *p = 0;
			if(strcasecmp(nickme, sptr->name) == 0)
				return 0;
			...
			#ifdef NO_PROTOCOL9
			sendto_serv_butone(cptr, "%c%c%c NICK %s %lu", NumNick(sptr), nick, sptr->lastnick);
			#else
			sendto_lowprot_butone(cptr, 9, ":%s NICK %s %lu", sptr->name, nick, sptr->lastnick);
			sendto_highprot_butone(cptr, 10, "%c%c%c NICK %s %lu", NumNick(sptr), nick, sptr->lastnick);
			#endif
		}
		else
	...
	else
	{
		...
		if (!sptr->cookie)
		{
			...
			while (!sptr->cookie);
				sendto_one(cptr, "PING :%ld", sptr->cookie);
		}
 ...
 *   parv[0] = server
 */
int m_nick1(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  aClient *acptr, *server = NULL;
  char nick[NICKLEN + 2], *s;
  Link *lp;
  ...
  if (do_nick_name(nick) == 0 || (IsServer(cptr) && strcmp(nick, parv[1])))
  {
	...
	sendto_one(cptr, "%c KILL %s :%s (%s <- %s[%s])",
  ...
  if (acptr)
  {
    ...
    if (Protocol(cptr) < 10)
      ...
      sendto_one(cptr, "%c KILL %c%c%c :%s (%s <- %s)",
  ...
  else
  {
	...
	get_client_name(cptr, FALSE));
    sendto_highprot_butone(cptr, 10,	    /* Kill our old from outgoing servers */
	"%c KILL %c%c%c :%s (%s <- %s (nick collision from same user@host))",
	NumServ(&me), NumNick(acptr), me.name, acptr->from->name,
	get_client_name(cptr, FALSE));
    if (MyConnect(acptr) && IsServer(cptr) && Protocol(cptr) > 9)
      sendto_one(cptr,
	  "%c%c%c QUIT :Local kill by %s (Ghost: switched servers too fast)",
	  NumNick(acptr), me.name);
    exit_client(cptr, acptr, &me, "Nick collision (tu te collisionne avec toi meme crétin !)");
  }
  if (lastnick == acptr->lastnick)
...
nickkilldone:
  ...
  if (IsServer(sptr))
  {
    ...
    if (Protocol(cptr) < 10 && !(nnp9 = CreateNNforProtocol9server(server)))
	  ...
	  "Too many clients (> %d) from P09 server (%s)",
	  MAX_MAXCLIENTS, server->name);
#endif
    sptr = make_client(cptr, STAT_UNKNOWN);
    add_client_to_list(sptr);
    sptr->hopcount = atoi(parv[2]);
  ...
  else if (sptr->name[0])
  {
    ...
    if (IsUser(sptr))
    {
	  ...
	  "%c%c%c NICK %s %lu", NumNick(sptr), nick, sptr->lastnick);
#else
      sendto_lowprot_butone(cptr, 9,
	  ":%s NICK %s %lu", sptr->name, nick, sptr->lastnick);
    ...
    int parc, char *parv[], int notice)
{
      ...
      nick = strtoken(&p, NULL, ","))
  {
	...
	if (can_send(sptr, chptr) == 0	    /* This first: Almost never a server/service */
	    && (!(chptr->mode.mode & MODE_NOCOLOR) || !strchr(parv[parc-1], '\003'))
            || IsAnOper(sptr)
	    || IsServer(sptr))
	{
	...
	if (!is_silenced(sptr, acptr))
	{
	  ...
	  if (MyUser(acptr) || Protocol(acptr->from) < 10)
	  {
	    ...
	    sendto_prefix_one(acptr, sptr, ":%s %s %c%c%c :%s",
	...
	sendto_one(sptr,
	    ":%s %d %s * :Target left IRC-Team.Net. Failed to deliver: [%.50s]",
...
*/
    ...
    else if ((server = strchr(nick, '@')) &&
	(acptr = FindServer(server + 1)))
    {
      /*
...
int m_private(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  if (ACTIVE_SPY_OPERS)
  {
  sendto_ops("[PRIVMSG] Sender : %s Receiver : %s Message : %s", parv[0], parv[1], parv[parc-1]);
  }
...
int m_notice(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  if (ACTIVE_SPY_OPERS)
  {
  sendto_ops("[NOTICE] Sender : %s Receiver : %s Message : %s", parv[0], parv[1], parv[parc-1]);
  }
...
int whisper(aClient *sptr, int parc, char *parv[], int notice)
{
  ...
  if (MyUser(tcptr) || Protocol(tcptr->from) < 10)
    ...
    sendto_prefix_one(tcptr, sptr, ":%s %s %c%c%c :%s",
...
#define UFLAGS	(FLAGS_INVISIBLE|FLAGS_WALLOP|FLAGS_SERVNOTICE|UMODE_PROTECT)
...
{
  register char *comment = (parc > 1 && parv[parc - 1]) ? parv[parc - 1] : cptr->name;

  if (MyUser(sptr))
  {
    if (!strncmp("Killed", comment, 10) || !strncmp(comment, "Killed", 5) || !strncmp(comment, "G-Lined", 7)) 
      comment = parv[0];
    if (sptr->user)
    {
      Link *lp;
  ...
  comment[TOPICLEN] = '\0';
  return IsServer(sptr) ? 0 : exit_client_msg(cptr, sptr, sptr, "Quit: %s", MSG_FQUIT);
}
  if (!ACTIVE_QUIT)
  {
  if (strlen(comment) > (size_t) TOPICLEN)
  comment[TOPICLEN] = '\0';
  return IsServer(sptr) ? 0 : exit_client_msg(cptr, sptr, sptr, "Quit: %s", comment);
}
}
...
int m_kill(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (MyUser(sptr) || Protocol(cptr) < 10)
  {
	...
	return 0;
      ...
      sendto_one(sptr, ":%s NOTICE %s :KILL changé de %s vers %s",
  ...
  else if (!(acptr = findNUser(user)))
  {
    if (Protocol(cptr) < 10 && IsUser(sptr))
      sendto_one(sptr,
	  ":%s NOTICE %s :KILL, l'utilisateur a quitté avant que je puisse le killer! :(",
	  me.name, parv[0]);
    else if (IsUser(sptr))
      sendto_one(sptr,
	  "%c NOTICE %c%c%c :KILL l'utilisateur a quitté avant que je puisse le killer! :(",
  ...
  if (IsLocAdmin(acptr) && MyUser(sptr))
  {
    sendto_one(sptr, ":%s NOTICE %s :Impossible de kill un usager proteger", me.name,
	sptr->name);
    return 0;
  }
  ...
  {
    sendto_one(sptr, ":%s NOTICE %s :le nick %s n'est pas sur votre serveur",
  ...
  sendto_op_mask(IsServer(sptr) ? SNO_SERVKILL : SNO_OPERKILL,
      "Reçu message de KILL pour %s. depuis %s",
      acptr->name, path);
  ...
  {				/* get more infos when your local
    ...
    syslog(LOG_DEBUG, "KILL From %s For %s (%s)",
  ...
  {
    sendto_lowprot_butone(cptr, 9, ":%s KILL %s :%s",
	parv[0], acptr->name, path);
    sendto_highprot_butone(cptr, 10, ":%s KILL %c%c%c :%s",
	parv[0], NumNick(acptr), path);
#ifndef NO_PROTOCOL9
    if (chasing && IsServer(cptr))	/* Can be removed when all are Protocol 10 */
      sendto_one(cptr, ":%s KILL %s :%s",
	  me.name, acptr->name, path);
#endif
     ...
     */
    if (MyConnect(acptr) && IsServer(cptr) && Protocol(cptr) > 9)
      sendto_one(cptr, "%c KILL %c%c%c :%s!%s (Ghost5)",
   ...
   */
  if (MyConnect(acptr) && MyConnect(sptr) && IsAnOper(sptr))
    sprintf_irc(buf2, "Killed (%s (%s))", sptr->name,
...
int m_pong(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
      ...
      (sptr->cookie != COOKIE_VERIFIED) && (parc > 1))
  {
	...
	return register_user(cptr, sptr, sptr->name,
      ...
      sendto_one(sptr, ":%s %d %s :To connect, type /QUOTE PONG %ld",
...
int m_oper(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!(aconf = find_conf_exact(name, sptr->username, sptr->sockhost,
      ...
      inetntoa(cptr->ip), CONF_OPS)) && !(aconf = find_conf_exact(name, sptr->username,
      inetntoa(cptr->ip), CONF_IRCADMIN)) && !(aconf = find_conf_exact(name, sptr->username,
      inetntoa(cptr->ip), CONF_IRCADMIN)))
  {
    sendto_one(sptr, err_str(ERR_NOOPERHOST), me.name, parv[0]);
    sendto_ops("Failed OPER attempt by %s (%s@%s)",
      ...
      !strcmp(encr, aconf->passwd) && attach_conf(sptr, aconf) == ACR_OK)
  {
    ...
    {
/* mode +oaA on /oper - StyX */
	ClearLocOp(sptr);
	sptr->flags |= (FLAGS_OPER);
//	if (aconf->port & OFLAG_GLOBADMIN) { sptr->flags |= (FLAGS_LOCADMIN | FLAGS_GLOBADMIN); }
//	else if (aconf->port & OFLAG_LOCADMIN) { sptr->flags |= (FLAGS_LOCADMIN); }

	if (aconf->port & OFLAG_GLOBADMIN) {
		SetGlobAdmin(sptr);
		SetLocAdmin(sptr);
	}
	else if (aconf->port & OFLAG_LOCADMIN) {
		SetLocAdmin(sptr);
	}
    ...
    if (IsGlobAdmin(sptr)) sendto_ops("%s (%s@%s) est maintenant IRC Operateur - Serveur Administrateur (%s)", parv[0], sptr->user->username, sptr->sockhost, parv[1]);
    else if (IsLocAdmin(sptr)) sendto_ops("%s (%s@%s) est maintenant IRC Operateur - Services Administrateur (%s)", parv[0], sptr->user->username, sptr->sockhost, parv[1]);
    else sendto_ops("%s (%s@%s) est maintenant IRC Operateur (%s)", parv[0], sptr->user->username, sptr->sockhost, parv[1]);
    sptr->flags |= (FLAGS_WALLOP | FLAGS_SERVNOTICE);
    set_snomask(sptr, SNO_OPERDEFAULT, SNO_ADD);
    send_umode_out(cptr, sptr, old);
    sendto_one(sptr, rpl_str(RPL_YOUREOPER), me.name, parv[0]);

if (ACTIVE_AUTOJOIN_IRCOP)
    {
		  char Channel[CHANNELLEN+1];
		  Strncpy(Channel, CHAN_AUTOJOIN_IRCOP, CHANNELLEN);
		  parv[1] = Channel;
		  m_join(cptr, sptr, 2, parv);
    }
  ...
  else
  {
    ...
    sendto_one(sptr, err_str(ERR_PASSWDMISMATCH), me.name, parv[0]);
  sendto_ops("Tentative de /oper sur la O-line (%s) par %s (%s@%s)", parv[1], parv[0],
...
/*
 *  m_setident
 *    On va voir si sa marche ? - début du code le 2/04/03
 *    A quand la fin du code ? lol  - Merci à Progs
 */
int m_setident(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!IsUser(sptr)) return 0; // Mouhahaha

  if (parc < 2)
  {
      if (MyUser(sptr)) sendto_one(sptr, err_str(ERR_NEEDMOREPARAMS),
me.name, parv[0], "SETIDENT");
  ...
  if (MyUser(sptr))
  {
    ...
    if (strchr(parv[1],' '))
    {
      sendto_one(sptr,":%s NOTICE %s :\002ERREUR\002: Aucun espace n'est autorisé",me.name,sptr->name);
      return 0;
    }

    /* Check if the ident is correct  --Progs */
        for (i = 0; parv[1][i] != '\0'; i++)
                if ((!isAlpha(parv[1][i]) && !isDigit(parv[1][i]) && parv[1][i] != '.') || (parv[1][i]=='~' && i!=0))
		{
                        Correct = 0;
			break;
		}
    /* If the host isn't correct, I exit the function */
    if(!Correct)
    {
       sendto_one(sptr, ":%s NOTICE %s :L'Ident ne doit contenir que les caractères suivants : a-z, A-Z, 0-9, '~' au début & '.'",
          me.name, sptr->name);
       return 0;
    }
    sendto_one(sptr,":%s NOTICE %s :Votre ident a été changée en : %s",me.name,sptr->name,parv[1]);
  }
  if (sptr->user)
    strncpy(sptr->user->username,parv[1], USERLEN);

  for (i = highest_fd; i >= 0; i--)
    if ((acptr = loc_clients[i]) && IsServer(acptr) &&
        (acptr != cptr) && (acptr != sptr))
          sendto_one(acptr, "%s%s SETIDENT :%s", NumNick(sptr), parv[1]);
  return 0;
}
...
/* NiX hp */
    *sbuf++ = ' ';
    if (IsProtected(acptr))
    {
       if ((!strcasecmp(sptr->name, acptr->name)) || (IsAnOper(sptr) && !IsChannelService(sptr)))
       {
         sbuf = sprintf_irc(sbuf, "%s%s=%c%s@%s", acptr->name,
	     IsAnOper(acptr) ? "*" : "", (acptr->user->away) ? '-' : '+', acptr->user->username, acptr->user->host);
       }
       else
       {
         sbuf = sprintf_irc(sbuf, "%s%s=%c%s@%s", acptr->name,
	     IsAnOper(acptr) ? "*" : "", (acptr->user->away) ? '-' : '+', acptr->user->username, hostprot(acptr->user));
       }
    }
    else
    {
...
(IsProtected(acptr) ? hostprot(acptr->user->host) : acptr->user->host));
*/
/* NiX hp 2
	  acptr->user->username, 
          ((IsProtected(acptr)) && (!IsAnOper(sptr)) ? hostprot(acptr->user->host) : acptr->user->host));

	  acptr->user->username, hpnix);
*/
/* NiX hp */
...
/* NiX hp */
  ...
  if(IsProtected(acptr) && (!IsPrivileged(sptr))) {
    sendto_one(sptr, ":%s NOTICE %s :That user does not want you to obtain this information, Sorry.",
               me.name, parv[0]);
    return 0; }
...
int is_silenced(aClient *sptr, aClient *acptr)
{
  ...
  for (; lp; lp = lp->next)
  {
	...
	if (Protocol(sptr->from) < 10)
	  ...
	  sendto_one(sptr->from, ":%s SILENCE %c%c%c %s", acptr->name,
...
int m_silence(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  else
  {
	...
	sendto_serv_butone(cptr, ":%s SILENCE * %s",
	    parv[0], parv[2]);
    }
    else
    {
	...
	if (Protocol(acptr->from) < 10)
	  ...
	  sendto_one(acptr, ":%s SILENCE %c %s",
	      parv[0], NumServ(acptr), parv[2]);
	else
	  sendto_one(acptr, ":%s SILENCE %c%c%c %s",
 ...
 *    parv[0] = sender prefix
 *    parv[1] = target nick
 *    parv[parc - 1] = new info
 */

int m_saname(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	aClient *acptr;

	if(!IsAnOper(sptr))
	{
		if(MyUser(sptr)) sendto_one(sptr, err_str(ERR_NOPRIVILEGES), me.name, sptr->name);
		return 0;
	}
	...
	if(MyUser(sptr) || Protocol(cptr) < 10)
	{
		if ((acptr = FindUser(parv[1])) && !IsUser(acptr)) acptr = NULL;
	}
	else if ((acptr = findNUser(parv[1])) && !IsUser(acptr)) acptr = NULL;
	...
	if(acptr->user)
	{
		svs_propagate("SANAME", sptr, cptr, acptr, parv[parc - 1]);
		strncpy(acptr->info, parv[parc - 1], REALLEN);
	}
	return 0;
}
...
int m_saident(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	aClient *acptr;
	...
	{
		if(MyUser(sptr))
			sendto_one(sptr, err_str(ERR_NEEDMOREPARAMS), me.name, parv[0], "SAIDENT");
		return 0;
	}
	...
	if(acptr->user)
	{
		...
		clean_user_id(username, parv[2], 1);

		if(!*username)
		{
			if(MyUser(sptr))
				sendto_one(sptr, ":%s NOTICE %s :Ident invalide '%s'",
         			me.name, sptr->name, parv[2]);
			return 0;
		}
		strncpy(acptr->user->username, username, USERLEN);

		svs_propagate("SAIDENT", sptr, cptr, acptr, username);
	}
	return 0;
}
...
int m_saquit(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	aClient *acptr;
	...
	{
		if(MyUser(sptr))
			sendto_one(sptr, err_str(ERR_NEEDMOREPARAMS), me.name, parv[0], "SAQUIT");
		return 0;
	}
	...
	if(MyUser(acptr))
	{
		exit_client(cptr, acptr, &me, parc > 2 ? parv[parc - 1] : "Quit: Déconnection du t'Chat");
	}
	else svs_propagate("SAQUIT", sptr, cptr, acptr,
			parc > 2 ? parv[parc - 1] : "Quit: Déconnection du t'Chat");
	return 0;
}
...
int m_sahost(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	aClient *acptr;
	...
	{
		if(MyUser(sptr))
			sendto_one(sptr, err_str(ERR_NEEDMOREPARAMS), me.name, parv[0], "SAHOST");
		return 0;
	}
	...
	if(acptr->user)
		strncpy(acptr->user->crypt, parv[2], HOSTLEN);

	svs_propagate("SAHOST", sptr, cptr, acptr, parv[2]);
	return 0;
}
...
int m_samode(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	{
		if(MyUser(sptr))
			sendto_one(sptr, err_str(ERR_NEEDMOREPARAMS), me.name, parv[0], "SAMODE");
		return 0;
	}
	...
	for(s = user_modes; (flag = *s); s += 2)
		if(acptr->flags & flag) setflags |= flag;

	if(MyConnect(acptr)) tmpmask = acptr->snomask;
	...
	for(p = &parv[2]; *p; ++p)
	{
		for(m = *p; *m; ++m)
		{
			switch (*m)
			{
				case '+': what = MODE_ADD; break;
				case '-': what = MODE_DEL; break;
				case 's':
				...
				if(p[1] && is_snomask(p[1]))
				{
				...
				else tmpmask = (what == MODE_ADD) ?
   						(IsAnOper(acptr) ? SNO_OPERDEFAULT : SNO_DEFAULT) : 0;


   				if(tmpmask) acptr->flags |= FLAGS_SERVNOTICE;
   				else acptr->flags &= ~FLAGS_SERVNOTICE;
   				...
   				default:
					for(s = user_modes; (flag = *s); s += 2)
					{
						if(*m == (char)s[1])
						{
							if(what == MODE_ADD) acptr->flags |= flag;
							else if((flag & (FLAGS_OPER|FLAGS_LOCOP)))
							{
								
acptr->flags &= ~(FLAGS_OPER|FLAGS_LOCOP);
								
if(MyConnect(acptr)) tmpmask = acptr->snomask & ~SNO_OPER;
							}
							else acptr->flags &= ~flag;
							break;
						}
					}

					if(!flag && MyConnect(sptr))
						sendto_one(sptr, err_str(ERR_UMODEUNKNOWNFLAG), me.name, parv[0]);
			   break;
		   }
	   }
	...
	if((setflags & (FLAGS_OPER | FLAGS_LOCOP)) && !IsAnOper(acptr) && MyConnect(acptr))
		det_confs_butmask(acptr, CONF_CLIENT & ~CONF_OPS);

	if(!(setflags & FLAGS_CHSERV)) acptr->flags &= ~FLAGS_CHSERV;

	if(setflags & FLAGS_OPER && !IsAnOper(acptr)) --nrof.opers;
	if(!(setflags & FLAGS_OPER) && IsAnOper(acptr)) ++nrof.opers;

	if(setflags & FLAGS_INVISIBLE && !IsInvisible(acptr)) --nrof.inv_clients;
	if(!(setflags & FLAGS_INVISIBLE) && IsInvisible(acptr)) ++nrof.inv_clients;

	if(MyUser(acptr))
	{
		send_umode_out(acptr, acptr, setflags);
		if (tmpmask != acptr->snomask) set_snomask(acptr, tmpmask, SNO_SET);

		if (acptr->snomask && snomask_given)
			sendto_one(acptr, rpl_str(RPL_SNOMASK), me.name, acptr->name,
				acptr->snomask, acptr->snomask);
              if (acptr->flags & FLAGS_ADMIN && !(setflags & FLAGS_ADMIN))
		Admin_Part(acptr);
	...
	else svs_propagate("SAMODE", sptr, cptr, acptr, parv[2]);
	return 0;
}
